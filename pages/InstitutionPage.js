...
exports.InstitutionPage = class InstitutionPage {
  constructor(page) {
    this.page = page;
    this.login_url = "/institution/login";
    this.register_url = "/institution/register";
  }
  async goToInstitutionDashboard() {
    await this.page.goto("/institution");
  }
  ...
  async institutionRegister(institution_name, email, password, confirm_password) {
    await this.page.goto(this.register_url);
    await this.page.getByRole("textbox", { name: "Institution Name*" }).fill(institution_name);
    await this.page.getByRole("textbox", { name: "Email address*" }).fill(email);
    await this.page.getByRole("textbox", { name: "Password*", exact: true }).fill(password);
    await this.page.getByRole("textbox", { name: "Confirm password*" }).fill(confirm_password);
    await this.page.getByRole("button", { name: "Sign up" }).click();
    await expect(this.page).toHaveURL("/institution");
  }

  async institutionLogin(insitution_data) {
    // 1. Assign page from class property for cleaner code refactoring
    const page = this.page;

    // 2. Perform Login
    await page.goto(page.login_url);
    ...
    await page.waitForTimeout(5000); // Consider replacing with a specific waitForSelector for stability
    ...
    const country = page.getByRole("textbox", { name: "Select Country" });
    ...
    const c_state = page.getByText("Select State");
    await c_state.click();
    ...
    const city = page.getByText("Select City");
    await city.click();
